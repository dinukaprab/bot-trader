...
from utils.terminal import clear_console, hide_cursor
...
def connect_mt5_account(account_info):
    ...
    if not mt5.initialize():
        stop_spinner()
        print(f"\r{RED}❌ Initialization failed: {mt5.last_error()}{RESET}")
        print(f"\r{RED}Restart your MT5 Terminal and try again.{RESET}")
        return False

    # Initialization successful
    stop_spinner()
    print(f"\r{GREEN}✔ Successfully connected to MetaTrader 5{RESET}")
    ...
    if not mt5.login(login, password=password, server=server):
        stop_spinner()
        print(f"\r{RED}❌ Login failed: {mt5.last_error()}{RESET}")
        print(f"\r{RED}Restart your MT5 Terminal and try again.{RESET}")
        return False   

    # Login successful
    stop_spinner()
    print(f"\r{GREEN}✔ Successfully logged into your Account{RESET}")
    return True